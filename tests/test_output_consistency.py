        ...
        torch.testing.assert_close(output.references[0].sequences, torch.tensor(
            [[15496, 11, 616, 1438, 318, 449, 13, 41, 13, 53, 13, 447, 103, 290, 356, 423, 257, 1049, 6180, 13],
             [1212, 318, 1194, 6827, 780, 612, 373, 2147, 2642, 351, 340, 13, 447, 237, 198, 1532, 484, 547, 284,
              423]]))
        torch.testing.assert_close(output.metric_scores.scores,
                                   torch.tensor([[43.1201, 46.1530, 43.5142, 43.8980, 44.0345],
                                                 [57.1227, 57.2903, 54.9877, 57.1268, 56.8152]]), atol=1e-4, rtol=1e-4)
        torch.testing.assert_close(output.metric_scores.scores_per_reference[0],
                                   torch.tensor([[100.0000, 23.5668, 28.9961, 29.3708, 33.6670],
                                                 [31.3434, 100.0000, 33.3013, 31.0525, 35.0679],
                                                 [31.0233, 26.7555, 100.0000, 30.4199, 29.3724],
                                                 [32.2136, 25.5630, 31.1885, 100.0000, 30.5251],
                                                 [35.0868, 27.4656, 28.6148, 29.0051, 100.0000]]), atol=1e-4, rtol=1e-4)
